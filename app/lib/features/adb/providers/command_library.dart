...
class CommandCategory {
  final String name;
  final String icon;
  final List<AdbCommand> commands;
...
class CommandLibrary {
  ...
  static const screenOperationCategory = CommandCategory(
    ...
    commands: [
      ...
      AdbCommand(
        name: '开始录屏',
        description: '开始屏幕录制',
        command: 'screenrecord /sdcard/video.mp4',
        category: '屏幕操作',
        dangerLevel: DangerLevel.low,
      ),
      AdbCommand(
        name: '停止录屏',
        description: '停止屏幕录制',
        command: 'pkill -l INT screenrecord',
        category: '屏幕操作',
        dangerLevel: DangerLevel.low,
      ),
      AdbCommand(
  ...
  static const networkCategory = CommandCategory(
    ...
    commands: [
      AdbCommand(
        name: 'WiFi 信息',
        description: '获取 WiFi 连接信息',
        command: 'dumpsys wifi | grep "Wi-Fi"',
        category: '网络',
        dangerLevel: DangerLevel.low,
      ),
      AdbCommand(
        name: 'IP 地址',
        description: '获取设备 IP 地址',
  ...
  /// 获取所有命令（平铺）
  static List<AdbCommand> getAllCommands() {
    return getAllCategories().expand((c) => c.commands).toList();
  }

  /// 根据名称查找命令
  static AdbCommand? findCommand(String name) {
    try {
  ...
  /// 根据分类获取命令
  static List<AdbCommand> getCommandsByCategory(String category) {
